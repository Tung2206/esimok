...
// Validate: object_id
if (!empty($object_id)) {
    $category_list  = get_the_terms($object_id, 'category');
    $category_first = !empty($category_list) ? $category_list[0] : null;

    if ($category_first) {
        $breadcrumb_arr[] = [
            'link'  => get_term_link($category_first),
            'title' => $category_first->name
        ];
    }
    ...
    if (is_single()) {
        ...
        $title_breadcrumb = get_field('_esimok_title_breadcrumb');
        ...
        if (count($breadcrumb_arr) >= 2) {
            $breadcrumb_arr[1]['class'] = 'active';
        }
    }
}

// Load CSS
wp_enqueue_style('esimok-vn-breadcrumb');
?>
<div class="esimok-breadcrumb mb-0">
    <div class="container">
        <div class="relative">
            <div class="row">
                <div class="col-md-12">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <?php foreach ($breadcrumb_arr as $key => $value) :
                                if (empty($value['title'])) continue;

                                $comma_last = ($key == count($breadcrumb_arr) - 1) ? '' : ',';
                                $schemas .= '{';
                                $schemas .= '"@type": "ListItem",';
                                $schemas .= '"position": ' . ($key + 1) . ',';
                                $schemas .= '"item":';
                                $schemas .= '{';
                                $schemas .= '"@id": "' . $value['link'] . '",';
                                $schemas .= '"name": "' . $value['title'] . '"';
                                $schemas .= '}';
                                $schemas .= '}';
                                $schemas .= $comma_last;
                                $class_temp = 'breadcrumb-item';
                                if ($key == count($breadcrumb_arr) - 1) $class_temp .= ' active ';
                                if (isset($value['class'])) $class_temp .= ' ' . $value['class'];

                                if (empty($value['link'])) : ?>
                                    <li class="<?php echo $class_temp; ?>"><?php echo $value['title']; ?></li>
                                <?php else : ?>
                                    <li class="<?php echo $class_temp; ?>">
                                        <a class="text-decoration-none" href="<?php echo $value['link']; ?>">
        ...
        <script type="application/ld+json">
            {
                ...
                "@type": "BreadcrumbList",
                "itemListElement": [<?php echo $schemas; ?>]
            }
        </script>
    </div>
</div>